...
void segmented_select_pairs(const key_t *keys_in, key_t *keys_out,
    ...
    if (num_segments * num_elements == 0 || num_topk >= num_elements) return;
    if (num_topk > 256 || num_topk > (int)(0.5 * num_elements)) {
...
template <typename key_t, typename value_t, bool IS_DESCENDING, int KEYS_PER_ITEM, int BLOCK_THREADS>
void segmented_threshold_select_pairs_impl(
    const key_t *keys_in, key_t *keys_out,
    const value_t *values_in, value_t *values_out,
    int *num_valids,
    int num_segments, int num_elements, int num_topk,
...
#pragma unroll
            ...
            while (num_start < num_elements) {
                method.select_blocked(keys, values, sizeof(key_t) * 8, 0,
                                      num_topk, threshold, keys_temp, values_temp, &num_selected);
                info.barrier();
#pragma unroll
                for (int ITEM = 0; ITEM < KEYS_PER_ITEM; ++ITEM) {
                    int offset = lid * KEYS_PER_ITEM + ITEM;
                    if (offset < num_selected) {
                        ...
                        int offset_ = num_start + offset - num_selected;
                        if (offset_ < num_elements) {
                            keys[ITEM] = keys_in[b_offset + offset_];
                            values[ITEM] = values_in[b_offset + offset_];
                        } else {
                            keys[ITEM] = 0;
                        }
                    }
                }
                num_start += SelectMethod::PROCESSING_LENGTH - num_selected;
                info.barrier();
            }

            method.select_blocked(keys, values, sizeof(key_t) * 8, 0,
                                  num_topk, threshold, keys_out + b_offset, values_out + b_offset, &num_selected);
            if (lid == 0) num_valids[b] = num_selected;
        });
    if (l->is_sync_mode()) l->stream_sync();
}

template <typename key_t, typename value_t, bool IS_DESCENDING>
inline void segmented_threshold_select_pairs_(const key_t *keys_in, key_t *keys_out,
                                              const value_t *values_in, value_t *values_out,
                                              int *num_valids, int num_segments,
                                              int num_elements, int num_topk, key_t threshold) {
    if (num_elements > 2048) {
        segmented_threshold_select_pairs_impl<key_t, value_t, IS_DESCENDING, 4, 1024>(
            keys_in, keys_out, values_in, values_out, num_valids, num_segments, num_elements, num_topk, threshold);
    } else if (num_elements > 1024) {
        segmented_threshold_select_pairs_impl<key_t, value_t, IS_DESCENDING, 4, 512>(
            keys_in, keys_out, values_in, values_out, num_valids, num_segments, num_elements, num_topk, threshold);
    } else if (num_elements > 512) {
        segmented_threshold_select_pairs_impl<key_t, value_t, IS_DESCENDING, 4, 256>(
            keys_in, keys_out, values_in, values_out, num_valids, num_segments, num_elements, num_topk, threshold);
    } else if (num_elements > 256) {
        segmented_threshold_select_pairs_impl<key_t, value_t, IS_DESCENDING, 4, 128>(
            keys_in, keys_out, values_in, values_out, num_valids, num_segments, num_elements, num_topk, threshold);
    } else {
        segmented_threshold_select_pairs_impl<key_t, value_t, IS_DESCENDING, 4, 64>(
            keys_in, keys_out, values_in, values_out, num_valids, num_segments, num_elements, num_topk, threshold);
    }
}

template <typename key_t, typename value_t>
void segmented_threshold_select_pairs(const key_t *keys_in, key_t *keys_out,
                                      const value_t *values_in, value_t *values_out,
                                      int *num_valids, /* output valid number per num_segments */
                                      int num_segments, int num_elements, int num_topk,
                                      key_t threshold, bool is_descending = true) {
    CHECK_FAIL(num_topk <= 256, "num_topk should smaller than 256");
    CHECK_FAIL(num_segments * num_elements != 0);
    CHECK_FAIL(is_descending == true);
    segmented_threshold_select_pairs_<key_t, value_t, true>(
        keys_in, keys_out, values_in, values_out, num_valids,
        num_segments, num_elements, num_topk, threshold);
}
